...
app.get("/api/projects", authMiddleware.optionalAuth, async (req, res) => {
  ...
  if (req.user) {
    const likedProjects = await likesService.getLikedProjectsForUser(
      req.user.id
    );

    for (let project of projects) {
      project.isLiked = likedProjects.has(project.airtableId.toString());
    }
  }
...
app.get("/api/projects/:id", authMiddleware.optionalAuth, async (req, res) => {
  try {
    ...
    if (req.user) {
      project.isLiked = await likesService.hasUserLikedProject(
        req.user.id,
        project.airtableId.toString()
      );
    }
...
app.get("/api/user/profile", authMiddleware.verifyToken, async (req, res) => {
  try {
    // On recharge l'utilisateur complet uniquement ici où c'est nécessaire
...
app.put("/api/user/profile", authMiddleware.verifyToken, async (req, res) => {
  try {
    ...
    // Récupérer l'état actuel pour les valeurs par défaut
  ...
  async (req, res) => {
    try {
      ...
      if (error.message === "Like already exists") {
        return res
          .status(400)
          .json({ error: "You have already liked this project" });
      }
...
app.delete(
  ...
  async (req, res) => {
    try {
      await likesService.removeLike(req.user.id, req.params.projectId);
