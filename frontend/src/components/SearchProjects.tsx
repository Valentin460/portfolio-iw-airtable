...
const SearchProjects: React.FC<SearchProjectsProps> = ({ onProjectSelect }) => {
  ...
  useEffect(() => {
    // Recharger quand l'état d'auth change; l'api met en cache par token
    loadAllProjects();
  }, [user]);
    ...
    <div style={containerStyle}>
      ...
      {searchTerm && (
        <div style={{ ...messageStyle, marginBottom: "2rem" }}>
          {filteredResults.length} projet{filteredResults.length > 1 ? "s" : ""}{" "}
          trouvé{filteredResults.length > 1 ? "s" : ""} pour "{searchTerm}"
        </div>
      )}

      {filteredResults.length === 0 && searchTerm ? (
        <div style={messageStyle}>Aucun projet trouvé pour "{searchTerm}"</div>
      ) : (
        <div style={gridStyle}>
      ...
      {!searchTerm && allProjects.length > 0 && (
        <div style={{ ...messageStyle, fontSize: "1rem", color: "#888" }}>
          ...
          disponible{allProjects.length > 1 ? "s" : ""}. Tapez dans la barre de
